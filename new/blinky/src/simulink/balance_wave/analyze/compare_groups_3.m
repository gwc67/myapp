function compare_groups_3(root_path)
% COMPARE_GROUPS_3 对比多组实验数据，按target_speed分组合并
% 修复版：自动拆分同一文件中包含多个target_speed段的数据
% 修复：处理时间重复问题，确保interp1正常工作
%
% 输入:
%   root_path - 根目录路径
% 输出:
...
%   超出原始数据范围的部分用最后一次有效值填充（不是NaN）
    ...
    all_files = {};      % 每个group对应一个cell，里面是该group的所有segment
    all_target_speeds = [];
    ...
    for g = 1:num_groups
        ...
        for f = 1:length(file_list)
            ...
            try
                ...
                % 找到target_speed变化的位置
                ts_diff = diff(ts_col);
                change_indices = find(abs(ts_diff) > 0.5);  % 变化超过0.5认为是新段
                ...
                for seg_idx = 1:length(change_indices)
                    ...
                    seg_info.seg_num = seg_num * 1000 + seg_idx;  % 用组合编号确保排序
                    seg_info.group_name = group_name;
                    seg_info.data = seg_data;
                    seg_info.seg_idx = seg_idx;  % 段内序号
    ...
    for t = 1:num_targets
        ...
        for c = 1:length(compare_cols)
            ...
            for g = 1:num_groups
                ...
                % 找到该组中所有target_speed匹配的段（不再break）
                matched_segments = {};
                ...
                if length(matched_segments) > 1
                    fprintf('  %s: found %d segments for target_speed=%d, merging...\n', ...
                        group_name, length(matched_segments), target_val);
