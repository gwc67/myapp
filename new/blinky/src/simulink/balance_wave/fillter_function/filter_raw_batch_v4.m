function filter_raw_batch_v4(folder_path)
% FILTER_RAW_BATCH_V4 批量处理文件夹下的所有CSV文件
% 按 running_success_flag 和 target_speed 双重拆分
% 新增：导出 angle_error = target_angle - pitch 列
%
% 输入:
...
%   每行数据多出 angle_error 列
    ...
    time_col = 1;
    target_speed_col = 7;
    pitch_col = 4;          % pitch 列号
    target_angle_col = 8;   % target_angle 列号
    angle_error_col = 10; % angle_error 列号 (新增)
    ...
    for f = 1:num_files
        ...
        num_standard_in = 9;  % 输入文件标准列数 (不含angle_error)

        if num_cols < num_standard_in
            fprintf('  Skipped: %s has only %d columns (need %d)\n', csv_name, num_cols, num_standard_in);
            continue;
        elseif num_cols > num_standard_in
            data = data(:, 1:num_standard_in);
        end

        %% 4.2 新增：计算 angle_error 列并拼接
        % angle_error = target_angle - pitch
        angle_error_vec = data(:, target_angle_col) - data(:, pitch_col);
        % 拼接为10列: [原9列, angle_error]
        data = [data, angle_error_vec];

        %% 4.3 找到 running_success_flag == 1 的连续段
        flag = data(:, flag_col);
        valid = (flag == 1);
        ...
        %% 4.5 逐段处理：先按running_flag分段，再按target_speed拆分
        export_count = 0;
