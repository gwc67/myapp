...
static real_T xnrm2(int32_T n, const real_T x[8], int32_T ix0)
{
  ...
  /* End of Start for MATLABSystem: '<S58>/MATLAB System' */
  return y;
}
...
static void trisolve(const real_T A[4], real_T B_0[4])
{
  ...
  /* Start for MATLABSystem: '<S58>/MATLAB System' */
  for (b_j = 0; b_j < 2; b_j++) {
    int32_T jBcol;
    jBcol = (b_j << 1) - 1;
    for (b_k = 0; b_k < 2; b_k++) {
...
void blinky_step1(void)                /* Sample time: [0.005s, 0.0s] */
{
   ...
   */
  rtDW.Sum2 = rtU.speed_a_target - rtU.motor_a_actual_speed;
   ...
   *  Sum: '<S34>/SumD'
   */
  rtb_FilterCoefficient = (SPEED_Kd * rtDW.Sum2 - rtDW.Filter_DSTATE) * 100.0F;
  ...
  if (rtU.running_flag > 0) {
    rtb_Switch = (int16_T)floorf((SPEED_Kp * rtDW.Sum2 + rtDW.Integrator_DSTATE)
  ...
  /* Outputs for Enabled SubSystem: '<S57>/Correct1' incorporates:
   *  EnablePort: '<S58>/Enable'
   */
  /* MATLABSystem: '<S58>/MATLAB System' incorporates:
   *  Constant: '<S57>/R1'
  ...
  for (i = 0; i < 2; i++) {
    ...
    rtDW.K_idx_0 = rtDW.P_i[0] * rtDW.epsilon;
    rtDW.z_idx_0 = rtDW.P_i[1] * rtDW.epsilon;
    b_i = i << 1;
    rtDW.A[b_i] = 0.0;
    rtDW.epsilon = rtDW.P_i[i + 2];
    rtDW.K[b_i] = rtDW.P_i[2] * rtDW.epsilon + rtDW.K_idx_0;
    rtDW.K[b_i + 1] = rtDW.P_i[3] * rtDW.epsilon + rtDW.z_idx_0;
    rtDW.A[b_i + 1] = 0.0;
  }

  /* Start for MATLABSystem: '<S58>/MATLAB System' */
  rtDW.epsilon = rtDW.K[0];
  rtDW.K_idx_0 = rtDW.K[1];
  rtDW.z_idx_0 = rtDW.K[2];
  rtDW.K_idx_2 = rtDW.K[3];
  for (i = 0; i < 2; i++) {
    rtDW.z_idx_1 = rtDW.b_dHdx[i];
    b_i = i << 1;
    rtDW.rtb_DataTypeConversion2_idx_0 = rtDW.epsilon * rtDW.z_idx_1 +
      rtDW.A[b_i];
    rtDW.rtb_DataTypeConversion2_idx_1 = rtDW.A[b_i + 1] + rtDW.K_idx_0 *
      rtDW.z_idx_1;
    rtDW.z_idx_1 = rtDW.b_dHdx[i + 2];
    rtDW.A[b_i] = rtDW.z_idx_0 * rtDW.z_idx_1 +
      rtDW.rtb_DataTypeConversion2_idx_0;
    rtDW.A[b_i + 1] = rtDW.K_idx_2 * rtDW.z_idx_1 +
      rtDW.rtb_DataTypeConversion2_idx_1;
  }

  /* MATLABSystem: '<S58>/MATLAB System' */
  rtDW.K[0] = rtDW.A[0];
  rtDW.K[1] = rtDW.A[2];
  rtDW.K[2] = rtDW.A[1];
  rtDW.K[3] = rtDW.A[3];
   ...
   * */
  rtDW.K_idx_0 = -rtDW.Sy[0];
  rtDW.z_idx_0 = -rtDW.Sy[2];
  rtDW.K_idx_2 = -rtDW.Sy[1];
  rtDW.z_idx_1 = -rtDW.Sy[3];
  ...
  for (b_i = 0; b_i < 2; b_i++) {
    ...
    rtDW.A_b = rtDW.K_idx_0 * rtDW.epsilon;
    rtDW.A_p = rtDW.z_idx_0 * rtDW.epsilon;
    rtDW.epsilon = rtDW.b_dHdx[i + 1];
    rtDW.A[i] = rtDW.K_idx_2 * rtDW.epsilon + rtDW.A_b;
    rtDW.A[i + 1] = rtDW.z_idx_1 * rtDW.epsilon + rtDW.A_p;
    A_tmp = i + b_i;
    rtDW.A[A_tmp]++;
    ...
    rtDW.x[b_i] += rtDW.K[b_i + 2] * rtDW.rtb_DataTypeConversion2_idx_1 +
      rtDW.K[b_i] * rtDW.rtb_DataTypeConversion2_idx_0;
  }

  /* End of Outputs for SubSystem: '<S57>/Correct1' */
   ...
   *  Gain: '<S1>/Gain1'
   */
  rtY.pitch = 57.295779513082323 * rtDW.x[0];

  /* Outputs for Atomic SubSystem: '<S57>/Predict' */
  /* Start for MATLABSystem: '<S60>/MATLAB System' incorporates:
   *  Inport: '<Root>/gyro'
   */
  /*  x = [pitch; roll]  (2×1) */
  /*  u = [gx; gy]       (2×1) */
  rtDW.K_idx_0 = rtU.gyro[1] * 0.005;

  /* MATLABSystem: '<S60>/MATLAB System' incorporates:
   *  DataStoreRead: '<S60>/Data Store ReadX'
   */
  rtDW.z_idx_0 = rtDW.K_idx_0 + rtDW.x[0];

  /* Start for MATLABSystem: '<S60>/MATLAB System' incorporates:
   *  Inport: '<Root>/gyro'
   */
  rtDW.K_idx_2 = rtU.gyro[0] * 0.005;

  /* MATLABSystem: '<S60>/MATLAB System' incorporates:
   *  DataStoreRead: '<S60>/Data Store ReadX'
   */
  rtDW.z_idx_1 = rtDW.K_idx_2 + rtDW.x[1];
  ...
  /*  u = [gx; gy]       (2×1) */
  /*  2×1 */
  rtDW.b_dHdx[0] = (((rtDW.x[0] + rtDW.epsilon) + rtDW.K_idx_0) - rtDW.z_idx_0) /
    rtDW.epsilon;
  ...
  rtDW.b_dHdx[3] = (((rtDW.x[1] + rtDW.epsilon) + rtDW.K_idx_2) - rtDW.z_idx_1) /
    rtDW.epsilon;

  /* End of Outputs for SubSystem: '<S57>/Predict' */
  /* End of Outputs for SubSystem: '<Root>/task_5ms' */
  /*  x = [pitch; roll]  (2×1) */
  /*  u = [gx; gy]       (2×1) */
  /*  2×1 */
   ...
   */
  rtDW.Integrator_DSTATE += SPEED_Ki * rtDW.Sum2;
  rtDW.Integrator_PrevResetState = (int8_T)rtb_LogicalOperator;

  /* Update for DiscreteIntegrator: '<S34>/Filter' incorporates:
   *  DiscreteIntegrator: '<S39>/Integrator'
   */
...
void blinky_initialize(void)
{
  ...
  /* Start for DataStoreMemory: '<S57>/DataStoreMemory - P' */
  rtDW.P_i[0] = 1.0;
  rtDW.P_i[1] = 0.0;
  rtDW.P_i[2] = 0.0;
  rtDW.P_i[3] = 1.0;
