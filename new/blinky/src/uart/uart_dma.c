...
//会有一个函数进行给它填充数据的
static int s_uart_transmit_dma(uart_base_t* base,uint8_t* data_puc ,uint32_t len_ul)
{
    struct uart_dma_t* me = CONTAINER_OF(base, struct uart_dma_t, base);
...
{
    struct uart_dma_t *me = (struct uart_dma_t* )user_data;
...
const uart_ops_t uart_dma_ops_st = {
    .uart_transmit = s_uart_transmit_dma,
